...
# Take the "Mean and Std"-features, transform them as a vector to pass it later as labels for the dataset
...
# Pass the transformed labels to the "Mean and Std"-Dataset
names(meanstdonly) <- descriptivelabels
# Merge Subject, Described Activities and "Mean and Std"-Dataset 
#names(allsubjects) <- "Subject"
datasetfinal <- cbind(allsubjects, describedactivities,meanstdonly)%>% rename(Subject = V1, Activity = describedactivities)
